...
export default function FeaturedVoicesSection() {
    ...
    return (
        <section id="featured-voices" className="relative bg-white overflow-hidden" aria-label="Ambience reel and featured voices">
            <div
                aria-hidden
                className="pointer-events-none absolute -top-24 -right-20 w-[34rem] h-[34rem] rounded-full blur-3xl opacity-20"
                style={{ background: 'radial-gradient(closest-side, rgba(46,83,57,0.22), rgba(230,126,34,0.08), transparent)' }}
            />
            <div
                aria-hidden
                className="pointer-events-none absolute -bottom-24 -left-16 w-[26rem] h-[26rem] rounded-full blur-3xl opacity-10"
                style={{ background: 'radial-gradient(closest-side, rgba(230,126,34,0.16), rgba(46,83,57,0.08), transparent)' }}
            />
            ...
            <div className="max-w-7xl mx-auto px-4 py-24 md:py-28">
                <div className="grid md:grid-cols-12 gap-10 md:gap-12 items-start">
                    ...
                    <div className="md:col-span-7">
                        ...
                        <motion.div layout transition={layoutTransition} className="hidden sm:flex gap-3 h-[22rem]" onMouseLeave={() => setActive(1)}>
                            {voices.map((v, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        <Image src={v.image} alt={`${v.name} portrait`} fill className="object-cover" sizes="(min-width: 640px) 33vw, 100vw" />
                                        ...
                                        <motion.div initial={false} animate={{ opacity: expanded ? 1 : 0.3 }} transition={{ duration: 0.25, ease: 'easeOut' }} className="absolute inset-0 p-5 flex flex-col">
                        ...
                        <div className="sm:hidden grid grid-cols-1 gap-4">
                            {voices.map((v) => (
                                <article key={v.name} className="relative overflow-hidden rounded-2xl border border-gray-200 bg-gray-50 h-64">
